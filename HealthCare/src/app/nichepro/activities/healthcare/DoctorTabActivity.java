...
import app.nichepro.fragmenttabdoctor.queries.DoctorQueryFragment;
import app.nichepro.fragmenttabpatient.info.PatientInfoMyNotesFragment;
import app.nichepro.util.Constants;

public class DoctorTabActivity extends BaseDefaultFragmentActivity {
	/* Your Tab host */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mStacks.put(Constants.TAB_APPOINTMENTS, new Stack<Fragment>());
		mStacks.put(Constants.TAB_PATIENT, new Stack<Fragment>());
	...
	public void initializeTabs() {
		...
		spec = mTabHost.newTabSpec(Constants.TAB_APPOINTMENTS);
		spec.setContent(new TabHost.TabContentFactory() {
			public View createTabContent(String tag) {
				return findViewById(R.id.realtabcontent);
			}
		...
		spec = mTabHost.newTabSpec(Constants.TAB_PATIENT);
		spec.setContent(new TabHost.TabContentFactory() {
			public View createTabContent(String tag) {
				return findViewById(R.id.realtabcontent);
			}
		...
		public void onTabChanged(String tabId) {
			...
			if (mStacks.get(tabId).size() == 0) {
				...
				if (tabId.equals(Constants.TAB_HOME)) {
					pushFragments(tabId, new DoctorHomeFragment(), false, true);
				} else if (tabId.equals(Constants.TAB_INFO)) {
					pushFragments(tabId, new DoctorInfoFragment(), false, true);
				} else if (tabId.equals(Constants.TAB_APPOINTMENTS)) {
					pushFragments(tabId, new DoctorAppointmentsFragment(),
							false, true);
				} else if (tabId.equals(Constants.TAB_PATIENT)) {
					pushFragments(tabId, new DoctorPatientListFragment(),
							false, true);
				} else if (tabId.equals(Constants.TAB_QUERIES)) {
					pushFragments(tabId, new DoctorQueryFragment(), false, true);
				}
			} else {
				/*
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_MENU) {
			...
			if (fb.getClass().getSimpleName().compareTo("DoctorQueryFragment") == 0) {
				((DoctorQueryFragment) fb).myOnKeyDown(KeyEvent.KEYCODE_MENU);
